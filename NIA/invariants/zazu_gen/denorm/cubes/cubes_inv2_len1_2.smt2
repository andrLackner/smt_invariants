...
; 		 -3*c + k*n - k + 2*n
; 		 3*n^2 - k + 3*n
; 		 (4*k^2*n) * (4 - 18*c - 9*c*n - 5*k + 9*n + k^2)
;--------------------------------------------------------------------------
...
(assert 
	(exists
		...
		(and
			...
			(= (+ (- (+ (* (- 3) c) (* k n)) k) (* 2 n)) 0)
			(= (+ (- (* 3 n n) k) (* 3 n)) 0)
			(= (* 4 k k n (+ (- 4 (* 18 c) (* 9 c n) (* 5 k)) (* 9 n) (* k k))) 0)
			...
			(or
				...
				(not (= (* 4 (+ k m) (+ k m) (+ n 1) (+ (- 4 (* 18 (+ c k)) (* 9 (+ c k) (+ n 1)) (* 5 (+ k m))) (* 9 (+ n 1)) (* (+ k m) (+ k m)))) 0))
			)
		)
	)
)
